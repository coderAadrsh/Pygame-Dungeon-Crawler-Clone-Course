...
class Arrow(pygame.sprite.Sprite):
  def __init__(self,image,x,y,angle):
    ...
    #cal the horizontal and virtical speeds based on the angle
  ...
  def update(self, screen_scroll, obstacle_tiles,enemy_list):
    ...
    #check for collision between arrow and tile wallls
    for obstacle in obstacle_tiles:
      if obstacle[1].colliderect(self.rect):
        self.kill()

    #check if the arrow is gone off screen
...
class Fireball(pygame.sprite.Sprite):
  ...
  def update(self, screen_scroll, player):
    ...
    #check if the fireball is gone off screen
    if self.rect.right<0 or self.rect.left>constants.SCREEN_WIDTH or self.rect.bottom<0 or self.rect.top>constants.SCREEN_HEIGHT:
      self.kill()
